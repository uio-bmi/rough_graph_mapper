import os
from .util import split_sam_by_chromosomes, run_bwa_mem, run_hybrid_between_bwa_and_minimap, run_minimap2
...
class LinearToGraphMapper:
        def __init__(self, fasta_file_name, linear_reference_file_name, graph_dir, chromosomes, minimum_mapq_to_graphalign=60, write_final_alignments_to_file=None, n_threads=3,
                     skip_mapq_adjustment=False, only_run_minimap=False):
            ...
            if only_run_minimap:
                logging.warning("Only running minimap, not BWA-MEM (means one should expect a fast run with poor accuracy")
                run_minimap2(linear_reference_file_name, fasta_file_name, self.base_name + ".sam", arguments="-t %d -x sr -N 7 -a --MD" % n_threads)
                ...
                run_bwa_mem(linear_reference_file_name, fasta_file_name, self.base_name + ".sam", arguments="-D 0.05 -t %d -h 1000000" % n_threads)
